...
@frappe.whitelist()
def get_balance_sheet_prepared_data():
    try:
        """
        Returns parsed data + applied filter values from the latest
        Prepared Report for Balance Sheet.
        Detects gzip and plain JSON storage.
        """
        
        # Step 1: Get latest Prepared Report for Balance Sheet
        pr = frappe.get_all(
            "Prepared Report",
            filters={"report_name": "Balance Sheet"},
            fields=["name", "filters"],
        ...
        if not pr:
            return {
                "success": False, 
                "message": "No Prepared Report found for Balance Sheet. Please generate the report first."
            }
        ...
        frappe.log_error("get_balance_sheet_prepared_data", frappe.get_traceback())
        return {
            "success": False,
